...
async function authorize() {
  try {
    ...
    const response = await axios.get(
      `${issuer}/v1/authorize?client_id=${client_id}&response_type=code&response_mode=form_post&scope=openid offline_access device_sso ${scopes}
...
async function getToken() {
  try {
    ...
    console.log('Token Request Successfull for Native App 1!! . Okta-Request-ID - ', response.headers['x-okta-request-id']);
